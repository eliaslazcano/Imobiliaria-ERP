export default class StringHelper {
  ...
  /**
   * Aplica a mascara ideal no telefone de acordo com a quantidade de digitos. Funciona com fixo e celular, com ou sem DDD.
   * @param {string} phone
   * @returns {string}
   */
  static beautifyPhone(phone) {
    if (!phone) return phone;
    phone = phone.replace(/\D/g,'');
    if (!phone || phone.length < 8) return phone;
    if (phone.length === 8) return phone.substr(0,4) + '-' + phone.substr(4);
    if (phone.length === 9) return phone.substr(0,5) + '-' + phone.substr(5);
    if (phone.length === 10) return '(' + phone.substr(0,2) + ') ' + phone.substr(2,4) + '-' + phone.substr(6);
    if (phone.length === 11) return '(' + phone.substr(0,2) + ') ' + phone.substr(2,5) + '-' + phone.substr(7);
    return phone;
  }
   ...
   * @param {boolean} strict Utiliza um algoritmo morderno.
   * @return {string}
   */
  static removeAccents(text, strict = false) {
    if (strict) return text.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  ...
  /**
   * Realiza uma higienizacao padrao no texto, removendo acentos, espacos extras, virgula no final e muda para caixa-alta.
   * @param {string} text Texto a ser higienizado.
   * @param {boolean} toUpperCase Mudar para caixa-alta.
   * @param {boolean} removeAccents Remover acentos.
   * @returns {string}
   */
  static clean(text, toUpperCase = true, removeAccents = true) {
    if (!text || typeof text !== 'string') return text;
    text = text.trim().replace(/\s{2,}/g, ' ');
    if (toUpperCase) text = text.toUpperCase();
    if (removeAccents) text = this.removeAccents(text);
    if (text.slice(-1) === ',') text = text.substring(0, text.length - 1);
    return text;
  }

  /**
   * Obtem a extensao do arquivo atraves do nome informado.
  ...
  /**
   * Obtem o nome do arquivo (com extensao) atraves do caminho informado.
