...
export const createWebClient = (errorCallback, mensagemErroConexao = null) => {
  if (!mensagemErroConexao) mensagemErroConexao = `Seu dispositivo não obteve sucesso ao tentar estabelecer conexão.`;
  ...
  //Error Handle
  const onError = error => {
    if (error.code === 'ECONNABORTED') errorCallback('A conexão do dispositivo foi interrompida.');
